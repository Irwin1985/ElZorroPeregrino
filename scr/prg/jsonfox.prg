...
*-- FUNCTION __parse_expr
	HIDDEN FUNCTION __parse_expr AS Variant
		...
		lnLenExp = lnLenExp + 1 && El valor mÃ¡s el salto.
...
*-- FUNCTION __parse_string
	HIDDEN FUNCTION __parse_string AS MEMO
		...
		THIS.__eat_json(LEN(lcValue) + 3) && El nombre mÃ¡s los delimitadores '"'/'"' y una posicion mÃ¡s para saltarse el Ãºltimo ".
...
*-- getvalue(tnIndex AS INTEGER)
	FUNCTION getvalue(tnIndex AS INTEGER) HELPSTRING "Obtiene el contenido del array dado su Ã­ndice."
		TRY
